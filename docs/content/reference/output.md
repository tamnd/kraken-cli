...
describes what users get. Pick a format with `-o`, or let kraken choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
kraken <command> -o table   # aligned columns for reading
kraken <command> -o jsonl   # one JSON object per line, for piping
kraken <command> -o json    # a single JSON array
kraken <command> -o csv     # spreadsheet friendly
kraken <command> -o tsv     # tab-separated
kraken <command> -o url     # just the URL column
kraken <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
kraken <command> --fields id,title,url
```
...
```bash
kraken <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
kraken <command>            # a table, because this is a terminal
kraken <command> | wc -l    # JSONL, because this is a pipe
```
